...
const ProductList = ({ productList }: { productList: { [key: string]: product[] } }) => {
    ...
    const rightEvent = () => {
        if (target !== null && target.current?.scrollWidth !== undefined) {
            target.current?.scrollLeft < target.current?.scrollWidth &&
                target.current?.scrollTo({ left: target.current?.scrollLeft + 1783, top: 0, behavior: "smooth" });
        }
    }

    const leftEvent = () => {
        target.current?.scrollLeft !== 0 &&
            target.current?.scrollTo({ left: target.current?.scrollLeft - 1763, top: 0, behavior: "smooth" });
    }
    ...
    return <div>
        ...
        <div className={"productList"} id={list} ref={target}>
            {product.map((product: {}, i: number) => <Product key={i} product={product} list={list} />)}
        </div>
